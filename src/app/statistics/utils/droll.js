(function (root) {
  ...
  droll.sum = function (result, formula) {
    var pieces = null;
  ...
  droll.roll = function (formula) {
    ...
    return droll.sum(result, formula);
  };

  droll.reroll = function (formula, roll, rerollId) {
    var pieces = null;
    var result = new DrollResult();
    ...
    result.rolls = roll.rolls;
    rerollId.forEach(function(id) {
      result.rolls[id] = (1 + Math.floor(Math.random() * pieces.numSides));
    });
